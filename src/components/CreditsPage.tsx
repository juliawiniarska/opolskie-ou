...
function CreditsLandingPage({ texts, phone }: { texts: AcfData | null; phone: string }) {
  // LOGIKA SEO DLA STRONY GŁÓWNEJ KREDYTÓW
  const pageTitle = texts?.kredyty_meta_title || "Kredyty i Finansowanie – Oferty | Opolskie Ubezpieczenia";
  ...
  const helmetContent = (
    <Helmet defer={false}>
      ...
      <meta name="description" content={texts?.kredyty_hero_desc || "Porównaj najlepsze oferty kredytów hipotecznych, gotówkowych i konsolidacyjnych w Nysie."} />
    </Helmet>
  );
...
export default function CreditsPage() {
  ...
  // Dynamiczna podmiana tytułu w komponencie rodzica w razie braku ładowania z Helmeta
  useEffect(() => {
    if (!slug) return;
    const mapItem = CREDIT_MAPPING.find((c) => c.slug === slug);
    if (mapItem && texts) {
      const metaTitle = texts[`${mapItem.acfPrefix}_meta_title`] || `${texts[`${mapItem.acfPrefix}_title`] || 'Kredyt'} – Oferta | Opolskie Ubezpieczenia`;
  ...
  if (isLoading) {
    return (
      <>
        <Helmet defer={false}>
          <title>Wczytywanie... | Opolskie Ubezpieczenia</title>
        </Helmet>
        <PageLoader />
      </>
    );
  ...
  if (!mapItem) {
    return (
      <main className="bg-[#F5F1E8] pt-28 pb-16 min-h-screen">
  ...
  // LOGIKA SEO DLA SZCZEGÓŁÓW KREDYTU
  const metaTitle = texts?.[`${prefix}_meta_title`] || `${title || 'Kredyt'} – Oferta | Opolskie Ubezpieczenia`;
  
  const detailHelmetContent = (
    <Helmet defer={false} key={slug}>
      <title>{metaTitle}</title>
      <meta name="description" content={description ? description.substring(0, 160) : "Szczegóły oferty kredytowej w Opolskie Ubezpieczenia."} />
    </Helmet>
  );
